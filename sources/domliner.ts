...
export class DOMLiner {
  ...
  element<T extends keyof ElementTagNameMap>(
    tag: T,
    decorations?: DOMDecorations<ElementTagNameMap[T]>,
  ...
  element(
    tag: string | Element,
    decorations?: DOMDecorations<any>,
    inner?: any
  ) {
...
const globalLiner =
  typeof document !== "undefined" ? new DOMLiner(self.document) : undefined;

export function element<T extends keyof ElementTagNameMap>(
  tagName: T,
  decorations?: DOMDecorations<ElementTagNameMap[T]>,
  children?: (string | Node)[]
): ElementTagNameMap[T];
...
export function element(
  ...
  if (!globalLiner) {
    throw new Error(
      "You cannot use DOMLiner.element as there is no global `document` variable in this platform. Please construct a new DOMLiner instance: `new DOMLiner(doc)`"
    );
  }
